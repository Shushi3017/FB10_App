...
class UserRepository {
    ...
    suspend fun getUsers(): List<User> {
        ...
        if (!response.success)
            error(response.message ?: "Unable to load users")
    ...
    suspend fun getUser(id: Int): User {
        ...
        if (!response.success)
            error(response.message ?: "Unable to load user")
    ...
    suspend fun createUser(request: UserRequest) {
        ...
        if (!response.success)
            error(response.message ?: "Unable to create user")
    }
    ...
    suspend fun updateUser(id: Int, request: UserRequest) {
        ...
        if (!response.success)
            error(response.message ?: "Unable to update user")
    }
